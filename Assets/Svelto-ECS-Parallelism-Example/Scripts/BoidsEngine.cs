            ...
            public bool MoveNext()
            {
                ...
                for (int index = _start; index < count; index++)
                {
                    for (int j = 0; j < 4; j++)
                    {
                        ...
                        IBoidComponent boidEntityView = _entityViews[index].entityView;
                        ...
                        var x = (realTarget.x - _entityViews[index].position.x);
                        var y = (realTarget.y - _entityViews[index].position.y);
                        var z = (realTarget.z - _entityViews[index].position.z);

                        var sqrdmagnitude = x * x + y * y + z * z;
                        _entityViews[index].position.x = x * sqrdmagnitude;
                        _entityViews[index].position.y = y * sqrdmagnitude;
                        _entityViews[index].position.z = z * sqrdmagnitude;
#endif
#if FIRST_TIER_EXAMPLE
                        var position = _entityViews[index].entityView.position;

                        var direction = realTarget - position;
                        var sqrdmagnitude = direction.sqrMagnitude;

                        _entityViews[index].entityView.position = direction / (sqrdmagnitude);
#endif
                    }
                }
#if TURBO_EXAMPLE
